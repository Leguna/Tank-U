...
namespace TankU.Gameplay
{
    public class PlayerInputController : BaseController<PlayerInputController>
    {
        ...
        public override IEnumerator Initialize()
        {
            ...
            _playerInput._PlayerMapInput.attack.performed += (ctx) => OnFire(ctx, 0);
            _playerInput._PlayerMapInput.bomb.performed += (ctx) => OnBomb(ctx, 0);
            ...
            _playerInput._PlayerMapInput1.attack.performed += (ctx) => OnFire(ctx, 1);
            _playerInput._PlayerMapInput1.bomb.performed += (ctx) => OnBomb(ctx, 1);
        }

        private void OnBomb(UnityEngine.InputSystem.InputAction.CallbackContext ctx, int v)
        {
            Publish(new BombMessage(v));
        }
